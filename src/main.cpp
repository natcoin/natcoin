...
uint256 hashGenesisBlock("0xde5fbe30db0cf8a9f0a10a4d3b59475573ad7e5e6e520b1bb2cc0c014d87e5c2");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Natcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Natcoin Signed Message:\n";
...
{
    // Natcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Natcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Natcoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Natcoin: 2.5 minutes
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Natcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Natcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xde5fbe30db0cf8a9f0a10a4d3b59475573ad7e5e6e520b1bb2cc0c014d87e5c2");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1395829832;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085976389;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Natcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("NatcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("NatcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static NatcoinMiner(CWallet *pwallet)
{
    printf("NatcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("natcoin-miner");
    ...
    try { loop {
        ...
        printf("Running NatcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("NatcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&NatcoinMiner, pwallet));
}
